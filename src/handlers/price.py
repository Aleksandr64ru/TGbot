...
async def handle_price(message: Message):
    try:
        ...
        async with aiohttp.ClientSession() as session:
            ...
            if not coin_id:
                await message.answer("❌ Ошибка получения coin_id")
                return
            ...
            price = round(coin.get("current_price") or 0, 2)
            ...
            async with session.get(chart_url, params={"vs_currency": "usd", "days": 1}) as resp:
                chart_data = await resp.json()

            prices = []

            if isinstance(chart_data, dict):
                prices = [p[1] for p in chart_data.get("prices", []) if p]
